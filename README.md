...
- **Three transfer modes** — server upload, WebRTC P2P (relayed signaling, direct data), or fully local (mDNS discovery, no server).
...
| `-m, --mode` | `u` | Transfer mode: `u` = server upload, `p2p` = WebRTC |
...
# Upload a file
nullseal share ./report.pdf -t file -p hunter2
...
nullseal share "hello" -m p2p -p hunter2

# Fully local — no internet required, receiver discovers via mDNS
nullseal share "hello" -m p2p -n local -p hunter2
```
...
# Receive locally (pairs with 'share -n local')
nullseal get -n local -p hunter2

# Direct connect — skip mDNS, useful on networks that block mDNS
nullseal get -n local -a 192.168.1.42:5555 -p hunter2
```
